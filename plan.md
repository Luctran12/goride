...
| Test status | Full `./mvnw.cmd test` passed 338 tests on 2026-06-24; targeted `SecurityCorsIntegrationTests` passed 3 tests on 2026-06-28; backend CI workflow added locally, awaiting first GitHub run |
...
| Request tracing, error logging, CORS and health checks | HTTP correlation ID, response trace header/body, request completion logs, centralized exception/security logs, configurable CORS allowlist, Actuator health/liveness/readiness/info endpoints | `X-Request-Id`, all REST endpoints, CORS preflight, `/actuator/health`, `/actuator/health/liveness`, `/actuator/health/readiness`, `/actuator/info` | Logs method/path/status/duration without request bodies or secrets; unexpected 5xx errors retain stack traces; FE origins are allowlisted by config and `X-Request-Id` is exposed; readiness checks DB/Redis while liveness only reflects app state. |
...
| P1 | Production hardening | Request correlation, centralized error logging, configurable CORS allowlist and Actuator health/readiness endpoints are implemented; add centralized log shipping, metrics/tracing backend and rate limits | Deployment platform requirements | API has safe production defaults and operational visibility across instances; deployment can use public liveness/readiness probes and FE browser clients can use explicit allowed origins. |
...
| Phase 4 | Production readiness | Firebase secure credential loading, HTTP request correlation, centralized application error logging, configurable CORS allowlist and Actuator health/readiness endpoints implemented; log aggregation, metrics/tracing backend, environment profiles, rate limits and release SQL strategy remain. |
